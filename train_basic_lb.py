...
#****************************************calculating loss over train subset****************************
		...
		for batch in train_loader:
			...
			#forward pass
			out = model(img0,img1)
			loss_b1 = loss(out[:,n1:n2],label[:,n1:n2])
			loss_b2 = loss(out[:,n2:n3],label[:,n2:n3])
...
#****************************************calculating loss over val subset****************************
		...
		for batch in val_loader:
			...
			#forward pass
			out = model(img0,img1)
			#label = (label+offset.expand(label.size()))/4
			loss_b1 = loss(out[:,n1:n2],label[:,n1:n2])
			loss_b2 = loss(out[:,n2:n3],label[:,n2:n3])
			#total_loss_train += loss_b.item()
...
#****************************************Training one epoch*******************************************
		...
		for batch in train_loader:
			...
			label = batch[2].to(device)
			out = model(img0,img1)
			loss_b = model.module.learned_weighting_loss(loss(out[:,0:3],label[:,0:3]),loss(out[:,3:6],label[:,3:6]))
			#backward pass
			loss_b.backward()
			optimizer.step()
#****************************************************************************************************
